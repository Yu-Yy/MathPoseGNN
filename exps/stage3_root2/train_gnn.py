...
def main():
    ...
    with Engine(cfg, custom_parser=parser) as engine:
        ...
        optimizer1 = make_optimizer(cfg, model, num_gpu)
        scheduler1 = make_lr_scheduler(cfg, optimizer1)
        ...
        engine.register_state( # provide the SMAP info
            scheduler=scheduler1, model=model, optimizer=optimizer1)
        ...
        if engine.continue_state_object: 
            ...
            if cfg.MODEL.WEIGHT: # load the SMAP pretrained model
                ...
                model.module.preload(cfg.MODEL.WEIGHT) # just load the SMAP part .module.module.
        ...
        for iteration, (images, valids, labels, rdepth, camera_para, joints3d_global, joints3d_local, scale) in enumerate(
            ...
            loss_dict,_,_ = model(images, valids, labels, rdepth, camera_para, joints3d_global, joints3d_local, scale) #, optimizer2, scheduler2
            ...
            if cfg.RUN_EFFICIENT: # efficiency
                del images, valids, labels

            if engine.local_rank == 0:
                if iteration % 20 == 0 or iteration == max_iter:
                    log_str = 'Iter:%d, LR(1):%.1e, ' % (
                        iteration, optimizer1.param_groups[0]["lr"] / num_gpu)
            ...
            # if iteration % max_iter == 0:  # test and save
            #     # test on VAl
            #     mpjpe = generate_3d_point_pairs(model, None, vali_data_loader, cfg, logger, device,
