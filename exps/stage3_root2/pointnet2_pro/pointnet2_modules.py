...
class PointnetSAModuleVotes(nn.Module): # TODO: 
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        ----------
        xyz : torch.Tensor
            (B, N, 21) tensor of the xyz coordinates of the features
        features : torch.Tensor
        ...
        xyz_flipped = xyz[...,:21].transpose(1, 2).contiguous()  # 21 forwards 
        if inds is None:
            inds = pointnet2_utils.furthest_point_sample(xyz, self.npoint) # TODO: 
        else:
            assert(inds.shape[1] == self.npoint)
        new_xyz = pointnet2_utils.gather_operation(  # only the index gather operation
            xyz_flipped, inds
        ).transpose(1, 2).contiguous() if self.npoint is not None else None
        ...
        if not self.ret_unique_cnt:
            grouped_features, grouped_xyz = self.grouper(
                xyz_, new_xyz, features  # TODO:
            )  # (B, C, npoint, nsample)
        else:
            grouped_features, grouped_xyz, unique_cnt = self.grouper(
                xyz_, new_xyz, features
            )  # (B, C, npoint, nsample), (B,3,npoint,nsample), (B,npoint)
        #print('???')
        #print(grouped_features.shape)
        grouped_features = torch.cat([grouped_xyz[:, :3, :, :], grouped_features[:, 21:, :, :]], dim = 1).contiguous()
        #print(grouped_features.shape, grouped_xyz.shape)
...
class PointnetFPModule(nn.Module):
    ...
    def forward(
        ...
        unknown : torch.Tensor
            (B, n, 21) tensor of the xyz positions of the unknown features
        known : torch.Tensor
            (B, m, 21) tensor of the xyz positions of the known features
...
class PointnetSAModuleDebug(nn.Module): # TODO: 
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def __init__(
        ...
        self.grouper = pointnet2_utils.QuaryAndGather(radius, nsample,
            use_xyz=use_xyz, ret_grouped_xyz=True, normalize_xyz=normalize_xyz,
            sample_uniformly=sample_uniformly, ret_unique_cnt=ret_unique_cnt)
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        xyz_flipped = xyz[...,:21].transpose(1, 2).contiguous()  # 21 forwards 
        if inds is None:
            inds = pointnet2_utils.furthest_point_sample(xyz.contiguous(), self.npoint) # TODO: 
        else:
            assert(inds.shape[1] == self.npoint)
        new_xyz = pointnet2_utils.gather_operation(  # only the index gather operation
            xyz_flipped, inds
        ).transpose(1, 2).contiguous() if self.npoint is not None else None   # return this new_xyz
        # import pdb; pdb.set_trace()
        #print(new_xyz.shape)
        xyz_ = xyz.contiguous()
        grouped_idns = self.grouper(
            xyz_, new_xyz, features  # TODO:
        )  # (B, C, npoint, nsample)
        return inds, grouped_idns, new_xyz
